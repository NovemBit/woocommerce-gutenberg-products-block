...
const AttributeFilterBlock = ( {
	...
	const filterAvailableTerms = blockAttributes.queryType === 'and';
	const {
	...
	useEffect( () => {
		...
		const newOptions = attributeTerms
			.map( ( term ) => {
				...
				const displayName = decodeEntities( term.name );

				return {
					value: term.slug,
					name: displayName,
					code: term.metadata.color,
					label: (
						<Label
							name={ displayName }
	...
	const multiple = blockAttributes.queryType === 'or';
