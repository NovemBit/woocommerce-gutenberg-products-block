...
const cleanURLSearchParams = ( urlParams ) => {
	...
	urlParams.forEach( ( v, k ) => {
		...
		if (
			v == '' ||
			( k.includes( 'filter' ) && ! attributesParams.includes( k ) )
		) {
			keysForDel.push( k );
		}
	} );
	...
	useEffect( () => {
		...
		searchParams.forEach( ( value, key ) => {
			if ( value !== null && ! key.includes( '_qt' ) ) {
				if ( key === 'category' ) {
					const CATEGORY_OPTIONS = getSetting(
						'categoryOptions',
						[]
					);
					...
					const STOCK_STATUS_OPTIONS = getSetting(
						'stockStatusOptions',
						[]
					);
	...
	useEffect( () => {
		...
		searchParams.forEach( ( value, key ) => {
			if (
				...
				attributeObject[ 'attribute' ] = key.replace(
					'filter_',
					'pa_'
				);
			...
			if ( params.includes( queryStateElement ) ) {
				...
				if ( queryStateElement === 'attributes' ) {
					...
					for ( const attribute of queryState[ queryStateElement ] ) {
						let to_push_attr = attribute.attribute.replace(
							'pa_',
							'filter_'
						);
						let encoded_to_push_attr = encodeURIComponent(
							to_push_attr
						);
						//attribute
						if (
							! attributesParams.includes( encoded_to_push_attr )
						) {
							attributesParams.push( encoded_to_push_attr );
						}
						if (
							! attributesParams.includes(
								encoded_to_push_attr + '_qt'
							)
						) {
							attributesParams.push(
								encoded_to_push_attr + '_qt'
							);
						}
						searchParams.set(
							to_push_attr,
							decodeURIComponent( attribute.slug ).toLowerCase()
						);
						searchParams.set(
							to_push_attr + '_qt',
							attribute.operator
